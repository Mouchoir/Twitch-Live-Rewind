...
- Current version: `1.3.2` (see `manifest.json` / `manifest.firefox.json`)
...
5. Store submission itself is semi-automated via a GitHub Actions workflow (see §9) rather than fully manual — the agent can point the user at running it, but cannot trigger it end-to-end unattended (it requires a human `workflow_dispatch` trigger and a required-reviewer approval by design).
...
## 9. Store publish workflow (CI)

`.github/workflows/publish-stores.yml` uploads/publishes the extension to the Chrome Web Store and Firefox Add-ons (AMO). Key points for anyone (human or agent) touching this:

- **Manual trigger only** — `workflow_dispatch` with inputs `ref` (tag/branch to build) and `mode` (`dry-run` or `publish`). It is intentionally never wired to run automatically on tag push or release, since it publishes to public stores.
- **Gated by the `stores-production` GitHub environment**, which has a required reviewer configured in repo settings (Settings → Environments). The job pauses for approval before the environment's secrets are exposed to it, even after `workflow_dispatch` is fired.
- **Secrets live only in that environment** (not repo-wide secrets): `CHROME_CLIENT_ID`, `CHROME_CLIENT_SECRET`, `CHROME_REFRESH_TOKEN`, `CHROME_EXTENSION_ID`, `AMO_JWT_ISSUER`, `AMO_JWT_SECRET`. They are Google OAuth2 (Chrome Web Store Publish API, scope `https://www.googleapis.com/auth/chromewebstore`) and Mozilla AMO JWT API key/secret respectively. None of these are committed anywhere; they were generated by the repo owner directly in Google Cloud Console / AMO Developer Hub and entered via `gh secret set --env stores-production`.
- **`mode: dry-run` vs `publish` are not symmetric between stores**: Chrome's `dry-run` only uploads a draft version (visible in the dev dashboard, not public); `publish` also calls the publish endpoint to go live. Firefox has no separate "make public" step — `dry-run` signs the xpi on the `unlisted` channel (no public submission), `publish` submits on the `listed` channel straight into Mozilla's review queue.
- Always run `dry-run` for a version once before `publish`, to sanity-check the build artifacts without touching either public listing.
- **AMO has no true dry-run**: unlike Chrome (where a draft upload can be freely re-uploaded/overwritten), Mozilla permanently consumes a version number the moment it's submitted, even on the `unlisted` channel used for `dry-run`. An `unlisted` submission cannot later be "promoted" to `listed` — it stays self-distribution-only forever (visible as a `Self` badge on the AMO versions page, with no publish/promote action available for it). Concretely: never `dry-run` test with the exact version number you intend to ship — that number is burned for the `listed`/public channel once used. If this happens, bump to the next patch version and redo the full release (manifest bump → tag → release → workflow) before running `publish`.
- **Chrome locks the item while a submission is in flight**: once `publish` succeeds, the item stays in "pending review" (or "ready to publish") on Google's side, and any further `upload` attempt fails with `ITEM_NOT_UPDATABLE` until that review resolves (approved or rejected) — there's no way to force it. If you need to ship a Firefox-only or Chrome-only fix while the other store is mid-review, that's exactly why `publish-chrome` and `publish-firefox` are separate jobs sharing one `build` job rather than one linear sequence: a stuck/failed Chrome submission no longer blocks Firefox from publishing (and vice versa), and GitHub's "Re-run failed jobs" will only retry the store that actually failed once its own review clears.
- If secrets are ever rotated or suspected compromised: Chrome refresh token can be revoked at https://myaccount.google.com/permissions; AMO keys are regenerated (and the old pair invalidated) from the same Developer Hub page they were created on. Update the corresponding `stores-production` environment secret afterward — never store a copy anywhere else.
